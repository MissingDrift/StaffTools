...
public class PlayTime implements CommandExecutor {
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!u.hasPerm("stafftools.playtime")) {
        ...
        if (target == null) {
            u.msg(Messages.target_is_offline.replace("%name%", args[0]).replace("%prefix%", Messages.prefix));
            return true;
        }
        User t = new User(target);
        u.msg(Messages.playtime_message.replace("%time%", t.getPlaytime()).replace("%name%", t.getName()));
        return true;
    }
}
